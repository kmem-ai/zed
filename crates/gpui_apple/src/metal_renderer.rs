...
impl MetalRenderer {
    ...
    pub fn render_scene_to_surface(
        ...
        // 3. Encode the frame into the surface's texture — the same path a window
        //    frame takes: `render_frame` writes the instance data and hands the
        //    instance buffer back to the pool once the GPU is done with it.
        let command_buffer = self.render_frame(scene, target_texture, size)?;

        // Wait so the surface is fully rendered before a consumer samples it.
        // The pixels live in the IOSurface — no read-back.
        command_buffer.commit();
        command_buffer.wait_until_completed();

        // `cv_texture` is released only now, after the GPU finished writing the
        // surface it wraps.
        drop(cv_texture);
        Ok(pixel_buffer)
    }
    ...
    fn draw_surfaces(
        ...
        for (index, surface) in surfaces.iter().enumerate() {
            ...
            // These CVMetalTextures must outlive the draw call below (the command
            // encoder retains the underlying MTLTextures at encode time), so they
            // are bound in the loop-body scope, not the branch.
            let bgra_texture;
            let y_texture;
            let cb_cr_texture;
...
mod surface_roundtrip_tests {
    use super::*;
    use gpui::{BorderStyle, Corners, Edges, Hsla, Quad, bounds, solid_background};
