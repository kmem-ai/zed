...
impl Window {
    ...
    fn capture_view_scene(&mut self, view: AnyView, size: Size<Pixels>, cx: &mut App) -> Scene {
        ...
        // Lay the root out the way `draw_roots` lays out a window root: an auto-sized root
        // stretches to fill the capture size, so the capture matches the on-screen frame.
        self.invalidator.set_phase(DrawPhase::Prepaint);
        let scale_factor = self.scale_factor();
        let mut root = view.into_any_element();
        let root_layout_id = root.request_layout(self, cx);
        self.layout_engine
            .as_mut()
            .unwrap()
            .stretch_auto_size_to_fill(root_layout_id, size, scale_factor);
        ...
        // was allocated in (the element borrows arena memory). Exiting the scope hands back
        // the clear this capture owes; a capture nested inside a draw defers it to that draw.
